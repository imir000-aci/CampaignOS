...
initTelemetry({ serviceName: "activation-platform" });
...
app.get("/health", async () => ({ status: "ok", service: "activation-platform" }));

try {
  await app.listen({ port, host: "0.0.0.0" });
  console.log(`activation-platform listening on :${port}`);
} catch (err) {
  console.error(err);
  process.exit(1);
}
