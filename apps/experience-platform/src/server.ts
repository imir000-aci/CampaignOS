...
initTelemetry({ serviceName: "experience-platform" });
...
app.get("/health", async () => ({ status: "ok", service: "experience-platform" }));

try {
  await app.listen({ port, host: "0.0.0.0" });
  console.log(`experience-platform listening on :${port}`);
} catch (err) {
  console.error(err);
  process.exit(1);
}
