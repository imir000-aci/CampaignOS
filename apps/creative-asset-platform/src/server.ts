...
initTelemetry({ serviceName: "creative-asset-platform" });
...
app.get("/health", async () => ({ status: "ok", service: "creative-asset-platform" }));

try {
  await app.listen({ port, host: "0.0.0.0" });
  console.log(`creative-asset-platform listening on :${port}`);
} catch (err) {
  console.error(err);
  process.exit(1);
}
