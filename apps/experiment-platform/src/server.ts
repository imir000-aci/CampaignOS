...
initTelemetry({ serviceName: "experiment-platform" });
...
app.get("/health", async () => ({ status: "ok", service: "experiment-platform" }));

try {
  await app.listen({ port, host: "0.0.0.0" });
  console.log(`experiment-platform listening on :${port}`);
} catch (err) {
  console.error(err);
  process.exit(1);
}
