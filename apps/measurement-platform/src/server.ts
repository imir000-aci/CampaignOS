...
initTelemetry({ serviceName: "measurement-platform" });
...
app.get("/health", async () => ({ status: "ok", service: "measurement-platform" }));

try {
  await app.listen({ port, host: "0.0.0.0" });
  console.log(`measurement-platform listening on :${port}`);
} catch (err) {
  console.error(err);
  process.exit(1);
}
